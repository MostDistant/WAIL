...
/// Run the signaling server on the given listener.
///
/// Accepts WebSocket connections and relays signaling messages between
/// peers in the same room. This function loops forever.
        ...
        match signal {
            SignalMessage::Join {
                ...
                write
                    .lock()
                    .await
                    .send(Message::Text(list_msg.into()))
                    .await?;
                ...
                for (id, sender) in room_map.iter() {
                    if let Err(e) = sender
                        .lock()
                        .await
                        .send(Message::Text(join_msg.clone().into()))
                        .await
                    {
        ...
        if let Some(room_map) = rooms_lock.get_mut(rname) {
            ...
            for (id, sender) in room_map.iter() {
                if let Err(e) = sender
                    .lock()
                    .await
                    .send(Message::Text(leave_msg.clone().into()))
                    .await
                {
