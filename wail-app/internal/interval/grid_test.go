...
func TestGridAlignerNotReadyUntilAnchorAndOffset(t *testing.T) {
	...
	g.ObserveServerTime(1_000_000, 500_000)
	if !g.Ready() {
		t.Fatal("anchor + offset sample should make aligner Ready")
	}
}
...
func TestGridAlignerDelta(t *testing.T) {
	...
	g.ObserveServerTime(10_000_000, 8_000_000)
...
func TestGridAlignerOffsetMedianFilter(t *testing.T) {
	g := NewGridAligner()
	// Steady offset 2s, one wild outlier; the median must reject the outlier.
	samples := [][2]int64{
		{10_000_000, 8_000_000},
		{10_000_100, 8_000_100},
		{10_000_200, 8_000_200},
		{30_000_000, 8_000_300}, // outlier
		{10_000_400, 8_000_400},
	}
	for _, s := range samples {
		g.ObserveServerTime(s[0], s[1])
	}
	off, ok := g.OffsetUs()
	if !ok {
		t.Fatal("no offset after samples")
	}
	if off < 2_000_000 || off > 2_000_400 {
		t.Fatalf("offset = %d, want in [2000000, 2000400] (median, outlier rejected)", off)
	}
}
