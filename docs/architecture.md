...
- **Recv:** the app fans each stream's paced playout chunk to the Link Audio sink *and* to any connected recv plugin as `RemotePCM`. The plugin renders arrival order (FIFO — it has no beat stamps to render against), so chunks must be *delivered* at their stamped beat rather than cushion-ahead of it: the IPC sink holds each chunk in a timed-release queue (`internal/emit.DueQueue`) that the emit loop flushes every tick, releasing a chunk when its beat arrives (minus a small fixed lead, `ipcLeadMs` = 20ms, for tick/socket jitter). Feeding the plugin the cushion-ahead stream directly made all remote audio play ~cushion late on this path. The plugin routes each stream to one of 16 stereo output ports and renames it live to `{peer} · {stream}` via CLAP's `RESCAN_NAMES` (allowed while active).
