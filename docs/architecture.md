...
- **Send:** the plugin taps its track's audio into a lock-free ring; an IPC thread streams `RawPCM` (float32 + a monotonic frame counter + a PLAYING flag) to the app. `ipcCaptureSource` anchors that frame counter to the app's local Link clock once and extrapolates by sample count — reproducing the sample-accurate begin-beat the Link Audio path gets from `Source.BeginBeats` (the DAW is a Link *sync* peer, so it shares the timeline). From there the normal capture → assemble → Opus → WAIF → relay path runs unchanged. The plugin also reports its DAW track's name (host `clap.track-info`, re-queried on the host's `changed()` callback) as a `TrackName` IPC frame — once after connect and on every rename — which the app uses as the stream's default label (`SetPluginSourceName`), so receivers see e.g. "Bass DI" instead of "Plugin Send 0"; hosts without track-info keep the placeholder.
- **Recv:** the app fans each stream's paced playout chunk to the Link Audio sink *and* to any connected recv plugin as `RemotePCM`. The plugin is a **stamp-aligned renderer** — the Link Audio receiver contract ("the receiver's timeline aligns the buffer's audio to the local audio") applied to a peer that can't speak Link: each chunk carries the machine-monotonic-µs instant its first frame should play (the app converts the chunk's Link-beat stamp via `TimeAtBeat` minus a per-tick measured Link↔mono bridge offset — the app can read both clocks, the plugin can't). The IPC sink holds chunks in a timed-release queue (`internal/emit.DueQueue`) and the emit loop flushes them a small lead (`WAIL_IPC_LEAD_MS`, default 20ms, clamped 5–100) ahead of their beat; the lead is now *delivery margin only* (starvation floor ≈ DAW block + chunk + jitter), decoupled from playback offset. The plugin keeps a per-slot anchor ring (chunk start frame ↔ play-at µs, SPSC like the data ring) and every block advances the ring head to the frame due *now*: early (e.g. delivery lead) → pad silence, late → skip forward (late audio is dropped, never played off-grid), with a 32-frame deadband (~0.67ms) that absorbs clock jitter without clicking. Two alignment modes, chosen per block: when the host transport is rolling with a beats timeline, v2 chunks (`RemotePCM2`, which adds the chunk's Link beat to the frame) align **fractional beat phase** against the CLAP transport (`song_pos_beats` — fixed-point beats × 2³¹) — the host's own transport→sample mapping absorbs output latency, making placement sample-accurate relative to the DAW grid; otherwise the mono-µs target is used (constant output-path offset). Phase targets snap by whole beats to the µs target, resolving the ±0.5-beat ambiguity. Wire version is negotiated by role byte (`IPCRoleRecvV2`; the plugin falls back to the v1 role if the app drops a frameless v2 connection), and the pool fans each chunk out per-connection-version. Legacy apps (small interval-index in the stamp field, < 10⁹ µs) get FIFO playback. Alignment drops are counted and reported to the app via `WAIL_TAG_METRICS`. History: feeding the plugin the cushion-ahead stream directly made all remote audio play ~cushion late (fixed in #438); the fixed 20ms lead then showed up as ~20ms early (the lead was the offset under FIFO) — stamp alignment removes that coupling entirely. The plugin routes each stream to one of 16 stereo output ports and renames it live to `{peer} · {stream}` via CLAP's `RESCAN_NAMES` (allowed while active).
...
- **Entry conformance** (join/rejoin only): adopt the room tempo, measure δ, and `ForceBeatAtTime` the local session onto the room grid **only if |δ| > ~25 ms**. Mid-blip reconnects measure δ ≈ 0 and no-op; first joins and app restarts snap. The measurement, not the event type, forks the behavior — there is no `isReconnect` branch. Snaps never fire mid-session.
- **Gated grid slew** (steady state): when |δ| crosses the threshold mid-session (crystal drift ≈ 3 ms/min), nudge local tempo ≤0.3% until δ closes, then restore the exact room tempo. Gates: never within a few seconds of any tempo change (local user's hand or remote adoption), never in the settling window after an entry snap.
