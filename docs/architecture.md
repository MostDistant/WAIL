...
- **Recv:** the app fans each stream's paced playout chunk to the Link Audio sink *and* to any connected recv plugin as `RemotePCM`. The plugin renders arrival order (FIFO — it has no beat stamps to render against), so chunks must be *delivered* at their stamped beat rather than cushion-ahead of it: the IPC sink holds each chunk in a timed-release queue (`internal/emit.DueQueue`) that the emit loop flushes every tick, releasing a chunk when its beat arrives (minus a small lead for tick/socket jitter: `WAIL_IPC_LEAD_MS`, default 20ms, clamped 5–100). Feeding the plugin the cushion-ahead stream directly made all remote audio play ~cushion late on this path. Because the plugin renders arrival order, the lead IS the playback offset; its floor is the DAW's block pull (process() takes a whole block per call and pads silence when short) — ≈10ms at 128-sample buffers, ≈18ms at 512 — so sub-block alignment requires the plugin to render against the host transport instead of FIFO (planned). The plugin routes each stream to one of 16 stereo output ports and renames it live to `{peer} · {stream}` via CLAP's `RESCAN_NAMES` (allowed while active).
