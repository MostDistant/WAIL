...
- **Push to `main`** → `beta.yml` merges main into the long-lived `beta` branch, runs `knope prepare-beta` (`prerelease_label = beta`) to bump to `v4.2.0-beta.N` on that branch (zero-indexed — first is `beta.0`), tags it, and dispatches the same `release.yml`. A merge with no `feat:`/`fix:`/changeset since the last beta is skipped (no artifact burned). `release.yml` marks beta tags as GitHub prereleases and writes a **separate `wail-beta` Homebrew formula** — a beta never repoints the `wail` formula stable users track. Betas and stable share the production relay and real rooms, so wire/protocol changes must stay backward compatible for at least one cycle. `scripts/verify-beta-versioning.sh` (gated by `verify-release-config.yml`) proves the numbering without cutting a release.
...
- **Never run the *mutating* release workflows against the real repo/remote.** `knope prepare-release` / `prepare-beta` / `release` bump files, cut tags, force-push the `release` branch, and create GitHub releases — CI owns them, and a local run races or duplicates the pipeline. Read-only previewing is fine and encouraged: `knope --dry-run <workflow>`, or `scripts/verify-beta-versioning.sh`, which drives the real `prepare-beta` against a throwaway repo (no remote, nothing pushed) to prove beta numbering without cutting anything.
