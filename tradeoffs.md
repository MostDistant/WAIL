...
### Emit cushion default is 160ms (feed-ahead vs added latency)
**Status:** Decided (2026-07-21, field-validated)
**File:** `wail-app/audio_engine_real.go` (`emitCushionMs`, `engineCushionFrames`)
**Decision:** The emit cushion stamps audio this far ahead of "now", so it adds directly to a Link Audio subscriber's reported buffering while defending the emit loop against local scheduler stalls (notably the App-Nap-adjacent ones the tunable-knob change addressed). Live A/B on one machine (loopback): 40ms let Live's buffer meter swing 0→max (its receive path kept running dry — 40ms sits inside Live's own jitter), 80ms held steady, 160ms best absorbed tab-switch stalls and a peer's receive-side glitches. 160 is the shipped default; the added latency is inaudible against the ~1-interval (multi-second) playout hold. The practical ceiling is the subscriber's configured Link Audio Latency — a cushion at/beyond it asks the receiver to hold more future audio than budgeted (the reference renderer stalls on far-future buffers), so anyone running Live below ~160ms Link Latency should lower `WAIL_EMIT_CUSHION_MS` (clamped 10–500; the hard 500 cap is 100 five-ms chunks, the largest burst the SDK's 128-buffer sink queue reliably accepts). Revisit the floor once App Nap pinning is confirmed to hold underruns at zero across real multi-peer sessions.
