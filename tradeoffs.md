...
**Coverage lost, deliberately:** (1) the `clap.state` roundtrip (`test_state.c`) — it has no subject, since neither Link Bridge plugin implements `clap.state` or `clap.params`; (2) the plugin ⇄ app wire-contract tests — there is no wire contract any more, the plugins are ordinary Link peers.
