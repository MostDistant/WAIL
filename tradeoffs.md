...
### Link Bridge Recv re-activation is unverified on Windows
**Status:** Open (assertion skipped on Windows only; macOS + Linux assert it)
...
**Problem:** on Windows CI the post-re-activate rediscovery never completes — not at 20s, not at 60s, while the same runner passes every other case including the in-place-rename test. Re-activate builds a *fresh* Link peer, and in this test both peers live in one process, so the destroy/recreate may be disturbing shared multicast state on Winsock rather than exercising anything the product does.
**Decision:** assert on macOS/Linux, skip on Windows with a printed note. Inflating the bound further would only have hidden the question behind a longer wait (the suite already ran 236s of its 240s budget at 60s), and dropping the test would lose the coverage that does work. The `reactivate()` call still runs on Windows, so a crash or hang is caught there.
**Reopen when:** someone can put a DAW on a Windows host and bypass/re-enable Link Bridge Recv. If audio returns, this is a harness artifact and the skip can go; if it does not, it is a real Windows bug in the plugin and the fix is incomplete there.
