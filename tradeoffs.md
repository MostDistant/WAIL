...
**Context:** `lbr_deactivate` used to leave slots marked `assigned` with their channel names intact, so a host bypass/re-enable left the plugin silent forever behind correct-looking port names. Fixed by clearing the dedupe keys alongside the source. The regression test passes reliably on macOS locally.
