...
**Checking playback alignment** — The Debug tab's *Playback* section has three more controls. **Publish WAIL Metronome channel** exposes a `WAIL Metronome` Link Audio channel — a click on every beat, accented on bar downbeats, on WAIL's own beat grid; subscribe to it in your DAW and run your DAW's metronome to confirm the two line up (a flam means the grids disagree, and a glitchy click means the playback path is struggling). **Broadcast metronome to room** additionally streams that same click to everyone in the room as audio: each peer auto-publishes a `<you> · WAIL Metronome` Link Audio channel and hears the identical grid one interval late. It's independent of the local channel above — run either or both. **Emit cushion** is a slider (0–500 ms, default 0) for how far ahead of the playhead WAIL keeps each Link Audio sink fed. It defaults to 0 (minimal) because the cushion stamps audio slightly into the future and some receivers — notably the VOID plugin — stall on any feed-ahead. If your receiver instead runs dry (sink underruns climbing in the health counters below), raise it until the dropouts stop (Ableton Live's native Link Audio liked ~80 ms). The Debug tab also carries audio stats, per-peer relay RTT, local audio-health counters, and the session log.
