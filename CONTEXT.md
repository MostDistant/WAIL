...
6. **One app is the whole system.** Session orchestration, Link bridging, codec, and networking live in a single app per musician (GUI or headless). The optional Link Bridge (ADR-0007) is a narrow exception: it is plugin-resident only for Link Audio rendering (it is a Link peer — sink/source and timeline mapping). Room intelligence — codec, intervals, relay, room clock — never enters any plugin.
...
**Link Bridge**:
The Link-Audio-native CLAP plugin pair (ADR-0007), and the only bridge WAIL ships: each instance is a Link peer, making a Link-but-not-Link-Audio DAW a Link Audio citizen. Recv is a 16-port device auto-populated from room-published channels; Send publishes one channel per track instance, named from the track.
